...
public class BusinchanceSampleGPTaskManage {

    private static Logger logger = Logger.getLogger(AppContent.calculatescorelog);

	private static BusinchanceSampleGPTaskManage instance = new BusinchanceSampleGPTaskManage();
	public static BusinchanceSampleGPTaskManage getInstance(){
		return instance;
	}
	...
	public boolean sampleBusinScore(String source){
		...
		try {
            ...
            Page page=ProdSampleDao.getInstance(false).getGunpaiBusinchanceByPage(pageBean);
            ...
            if (obcList != null && obcList.size() > 0) {
				for (OnBusinChance busin : obcList) {
                    try{
                        ...
                        logger.error("滚排商机算分异常，OnBusinChance:"+busin);
                        e.printStackTrace();
                    }
				}
			}else{
				logger.error("没有滚排商机信息");
			}

            while (10000>= pageBean.getEndNo()&&pageBean.getCount()!=pageBean.getEndNo()){
                pageBean.setPage(pageBean.getNextPageNo());
                page=ProdSampleDao.getInstance(false).getGunpaiBusinchanceByPage(pageBean);
            ...
            logger.info("处理了"+count+"条滚排商机信息");

		} catch (Exception e) {
            logger.error("滚排商机算分处理异常"+e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
