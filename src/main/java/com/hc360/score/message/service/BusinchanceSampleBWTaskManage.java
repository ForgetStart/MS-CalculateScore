...
public class BusinchanceSampleBWTaskManage {

    private static Logger logger = Logger.getLogger(AppContent.calculatescorelog);

	private static BusinchanceSampleBWTaskManage instance = new BusinchanceSampleBWTaskManage();
	public static BusinchanceSampleBWTaskManage getInstance(){
		return instance;
	}
	...
	public boolean sampleBusinScore(String source){
		...
		try {
            ...
            Page page=ProdSampleDao.getInstance(false).getBiaowangBusinchanceByPage(pageBean);
            ...
            if (obcList != null && obcList.size() > 0) {
				for (OnBusinChance busin : obcList) {
                    try{
                        ...
                        logger.error("标王商机算分异常，OnBusinChance:"+busin);
                        e.printStackTrace();
                    }
				}
			}else{
				logger.error("没有标王商机信息");
                return false;
			}

            while (100000>= pageBean.getEndNo()&&pageBean.getCount()!=pageBean.getEndNo()){
                pageBean.setPage(pageBean.getNextPageNo());
                page=ProdSampleDao.getInstance(false).getBiaowangBusinchanceByPage(pageBean);
            ...
            logger.info("处理了" + count + "条标王商机信息");

		} catch (Exception e) {
            logger.error("标王商机算分处理异常"+e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
