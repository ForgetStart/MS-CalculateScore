...
public class BusinchanceSamplePTTaskManage {

    private static Logger logger = Logger.getLogger(AppContent.calculatescorelog);

	private static BusinchanceSamplePTTaskManage instance = new BusinchanceSamplePTTaskManage();
	public static BusinchanceSamplePTTaskManage getInstance(){
		return instance;
	}
	...
	public boolean sampleBusinScore(String source){
		...
		try {
            ...
            Page page=ProdSampleDao.getInstance(false).getPutongBusinchanceByPage(pageBean);
            ...
            if (obcList != null && obcList.size() > 0) {
				for (OnBusinChance busin : obcList) {
                    try{
                        ...
                        logger.error("普通商机算分异常，OnBusinChance:"+busin);
                        e.printStackTrace();
                    }
				}
			}else{
				logger.error("没有普通商机信息");
                return false;
			}

            while (150000>= pageBean.getEndNo()&&pageBean.getCount()!=pageBean.getEndNo()){
                pageBean.setPage(pageBean.getNextPageNo());
                page=ProdSampleDao.getInstance(false).getPutongBusinchanceByPage(pageBean);
            ...
            logger.info("处理了"+count+"条普通商机信息");

		} catch (Exception e) {
            logger.error("普通商机算分处理异常"+e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
