...
public class BusinchanceSampleJDBWTaskManage {

    private static Logger logger = Logger.getLogger(AppContent.calculatescorelog);

	private static BusinchanceSampleJDBWTaskManage instance = new BusinchanceSampleJDBWTaskManage();
	public static BusinchanceSampleJDBWTaskManage getInstance(){
		return instance;
	}
	...
	public boolean sampleBusinScore(String source){
		...
		try {
            ...
            Page page=ProdSampleDao.getInstance(false).getJDBiaowangBusinchanceByPage(pageBean);
            ...
            if (obcList != null && obcList.size() > 0) {
				for (OnBusinChance busin : obcList) {
                    try{
                        ...
                        logger.error("极度标王商机算分异常，OnBusinChance:"+busin);
                        e.printStackTrace();
                    }
				}
			}else{
				logger.error("没有极度标王商机信息");
			}

            while (40000>= pageBean.getEndNo()&&pageBean.getCount()!=pageBean.getEndNo()){
                pageBean.setPage(pageBean.getNextPageNo());
                page=ProdSampleDao.getInstance(false).getJDBiaowangBusinchanceByPage(pageBean);
            ...
            logger.info("处理了"+count+"条极度标王商机信息");

		} catch (Exception e) {
            logger.error("极度标王商机算分处理异常"+e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
